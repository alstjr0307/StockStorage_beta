...
class ForeignPost extends StatefulWidget {
  @override
  _ForeignPostState createState() => _ForeignPostState();
}

class _ForeignPostState extends State<ForeignPost>
    with AutomaticKeepAliveClientMixin<ForeignPost> {

  final BannerAd myBanner = BannerAd(
    adUnitId: 'ca-app-pub-6925657557995580/7828829800', //자신의 UnitID
    size: AdSize.banner,
    request: AdRequest(),
    listener: AdListener(),
  );
        ...
        actions: [
          if (token !=null)
            IconButton(icon: Icon(Icons.add), onPressed: () {
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) =>ForeignAddPost()
                  )
              );
          ...
          IconButton(icon: Icon(Icons.search), onPressed: () {
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) =>ForeignSearchPage()
                )
            );
          })
        ],
