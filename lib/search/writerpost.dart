...
class WriterPost extends StatefulWidget {
  ...
  const WriterPost({Key key, this.userID, this.nickname}) : super(key: key); //index = 게시물 번호
...
class _WriterPostState extends State<WriterPost>
  ...
  void _getMoreData(int index) async {
    ...
    if (!isLoading) {
      ...
      var url = "http://13.125.62.90/api/v1/BlogPosts/?owner=${widget.userID}&page=" +
          (index + 1).toString();
      print(url);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(widget.nickname+'님의 글'),
      ),
      body: Column(
        children: [
          adContainer ?? Container(),
