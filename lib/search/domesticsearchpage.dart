...
class _DomesticSearchPageState extends State<DomesticSearchPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text('국내주식 검색')),
        body: Column(
          children: [
            _SearchSection(),
            _buildList(),
      ...
      children: [
        ...
        IconButton(
            ...
            onPressed: () {
              ...
              else if (searchOption == '내용') _getMoreDataContent(page);
  ...
  void _getMoreDataContent(int index) async {
    ...
    if (!isLoading) {
      ...
      var url =
          "http://13.125.62.90/api/v1/BlogPosts/?category=D&contentsearch=${searchController.text}&page=" +
              (index + 1).toString();
      print(url);
  ...
  void _getMoreDataTitle(int index) async {
    ...
    if (!isLoading) {
      ...
      var url =
          "http://13.125.62.90/api/v1/BlogPosts/?category=D&titlesearch=${searchController.text}&page=" +
              (index + 1).toString();
      print(url);
      final response = await dio.get(url);
      maxpage = (response.data['count'] - 1) ~/ 10 + 1;
  ...
  void _getMoreData(int index) async {
    ...
    if (!isLoading) {
      ...
      var url =
          "http://13.125.62.90/api/v1/BlogPosts/?category=D&multisearch=${searchController.text}&page=" +
              (index + 1).toString();
      print(url);
      final response = await dio.get(url);
      maxpage = (response.data['count'] - 1) ~/ 10 + 1;
